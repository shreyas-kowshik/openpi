...
class RolloutSaverSubscriber(_subscriber.Subscriber):
    ...
    def __init__(
        ...
        self._fps = fps
        # When set, ``on_episode_end`` reads ``label_source.latest`` to (a)
        # write success/status as HDF5 attrs and (b) discard the rollout on
        # ``aborted`` / ``retry``. Subscriber-registration order must put the
        # labeler BEFORE this saver so ``latest`` is populated by the time we
        # finalize.
    ...
    def on_episode_start(self) -> None:
        # Per-episode dir name: second-resolution timestamp + episode index +
        # microseconds. The latter two are guards against collision when
        # rapid retries / short episodes complete within the same second
        # (timestamp-only names overwrote each other previously).
        now = datetime.now()
        stamp = now.strftime("%Y%m%d_%H%M%S")
        micros = f"{now.microsecond:06d}"
        ep_idx = (
            self._label_source._episode_index  # noqa: SLF001
            if self._label_source is not None
            else self._episode_count
        )
        self._episode_dir = self._output_dir / f"{self._run_tag}_{stamp}_ep{ep_idx:04d}_{micros}"
    ...
    def on_episode_end(self) -> None:
        ...
        # Discard aborted / retry rollouts entirely: no HDF5, no video files,
        # episode dir removed. The success-label subscriber's status is the
        # source of truth here; absence of a labeler means save unconditionally.
        label = self._label_source.latest if self._label_source is not None else None
        if label is not None and label.status in ("aborted", "retry"):
            logging.info(
                "RolloutSaver: discarding %s rollout (%d steps, %.1fs wall)",
                label.status, n_steps, duration_s,
            )
            try:
                self._video_writer.finish()
            except Exception:  # noqa: BLE001
                logging.exception("RolloutSaver: video writer finish failed during discard")
        ...
        self._save_hdf5(hdf5_path, video_info, n_steps, label)
    ...
    def _save_hdf5(
        ...
        with h5py.File(path, "w") as f:
            ...
            if label is not None:
                f.attrs["is_success"] = bool(label.is_success)
                f.attrs["status"] = label.status
                f.attrs["episode_index"] = int(label.episode_index)
                f.attrs["labeled_at_ns"] = int(label.labeled_at_ns)
