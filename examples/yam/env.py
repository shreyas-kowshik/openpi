...
class YamEnvironment(_environment.Environment):
    ...
    def __init__(
        ...
        # Optional callable: returns True when the runtime should mark the
        # current episode complete on the next step. The eval client wires
        # SuccessLabelSubscriber's mid-rollout `terminate_episode` flag here
        # so 1/0/2/r presses during a rollout short-circuit the episode loop.
        self._terminate_check = terminate_check
    ...
    def is_episode_complete(self) -> bool:
        if self._terminate_check is None:
            return False
        try:
            return bool(self._terminate_check())
        except Exception:  # noqa: BLE001
            logging.exception("terminate_check raised; treating as no-termination")
            return False
