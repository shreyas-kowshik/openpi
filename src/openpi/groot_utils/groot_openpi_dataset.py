...
# Alias used by visualization / inspection scripts.
get_ep_meta_for_episode = load_ep_meta
...
def get_reference_ep_meta(
    ...
    """Return the ep_meta of the first training episode after applying the same
    filters that ``GrootOpenpiSingleDataset`` uses, so the reference exactly
    matches the demo used for training."""
    if layout_and_style_ids is not None:
        object_categories = [obj_category] if obj_category else None
        filtered = get_scene_filtered_demos(
            dataset_path, layout_and_style_ids, num_demos,
            fixture_refs=fixture_refs,
            object_categories=object_categories,
        )
        return load_ep_meta(dataset_path, filtered[0])
    # Fallback when no scene filters are set — return first episode.
    episodes_path = dataset_path / "meta" / "episodes.jsonl"
    with open(episodes_path) as f:
        first = json.loads(f.readline())
    return load_ep_meta(dataset_path, first["episode_index"])
