...
class ResidualPolicyWrapper(_base_policy.BasePolicy):
    ...
    def __init__(
        ...
        self._action_dim = 14
        # The trainer's actor outputs (chunk_len * action_dim) flat; reshape.
        # Action width is tied to chunk_len, NOT query_freq: the actor predicts a
        # full chunk_len-step plan and the trainer executes only the first
        # query_freq of it before re-querying (query_freq is just the re-query
        # cadence / discount horizon). Keeping the actor head at chunk_len lets a
        # checkpoint trained at one query_freq be continued at another.
        self._actor_chunk_shape = (self._chunk_len, self._action_dim)
    ...
    def infer(self, obs: dict) -> dict:  # type: ignore[override]
        ...
        # The residual actor was built with sample_obs['base_action'] of length
        # chunk_len. The base policy emits a longer chunk (action_horizon, e.g.
        # 60 for YAM-arrange); truncate to chunk_len here so the actor's MLP
        # input dim matches what it was initialized with. Trainer-side already
        # does this in train_utils_yam_residual.py:626/630/811 — keep both
        # sides in sync.
        a_base_for_actor = a_base_norm[: self._chunk_len]
        ...
        actor_obs = self._build_residual_obs(obs, a_base_for_actor, vlm_embedding)
        # eval_actions returns (B, action_dim_flat); reshape to (chunk_len, 14).
    ...
    async def update_actor_params(self, params_pytree: Any, batch_stats: Any = None) -> dict:
        """Swap the residual actor's params (and optional batch_stats) atomically.

        ``batch_stats`` mirrors Flax ``TrainState.batch_stats``. When the actor has
        mutable collections (e.g. BatchNorm), the server must swap them in lockstep
        with the params, otherwise its actor's running statistics drift from the
        trainer's. Four cases:

        - server-has-stats AND client sends stats: structure-check + atomic swap.
        - server-has-stats AND client sends None: keep existing stats, warn once
          per connection (backward compat with older clients).
        - server-has-None AND client sends stats: raise — encoder mismatch between
          client and server.
        - server-has-None AND client sends None: params-only swap (no-op for stats).

        Caller holds the websocket; ``self._params_lock`` makes the actor swap
        observable to concurrent ``infer`` calls atomically.
        """
        current_params = self._agent._actor.params  # noqa: SLF001
        current_struct = jax.tree_util.tree_structure(current_params)
        ...
        server_bs = getattr(self._agent._actor, "batch_stats", None)  # noqa: SLF001
        batch_stats_updated = False
        new_batch_stats = None
        if batch_stats is not None and server_bs is None:
            raise ValueError(
                "Client pushed batch_stats but server actor has no batch_stats "
                "collection. Client/server disagree on the encoder (server is not "
                "configured with BatchNorm)."
            )
        if batch_stats is not None and server_bs is not None:
            server_bs_struct = jax.tree_util.tree_structure(server_bs)
            incoming_bs_struct = jax.tree_util.tree_structure(batch_stats)
            if server_bs_struct != incoming_bs_struct:
                raise ValueError(
                    f"Residual actor batch_stats structure mismatch on push. "
                    f"current={server_bs_struct}, incoming={incoming_bs_struct}"
                )
            new_batch_stats = jax.tree.map(jnp.asarray, batch_stats)
            batch_stats_updated = True
        elif batch_stats is None and server_bs is not None and not self._warned_missing_bs:
            logger.warning(
                "Client did not push batch_stats but server actor has BatchNorm "
                "collections; keeping existing batch_stats. Server-side BN running "
                "stats will drift from the trainer until pushes start including them."
            )
            self._warned_missing_bs = True

        new_params = jax.tree.map(jnp.asarray, params_pytree)
        async with self._params_lock:
            if batch_stats_updated:
                self._agent._actor = self._agent._actor.replace(  # noqa: SLF001
        ...
        while True:
            try:
                ...
                if method == "infer":
                    ...
                    batch_stats = message.get("batch_stats")
                    result = await self._policy.update_actor_params(params, batch_stats=batch_stats)
                else:
...
def _build_residual_agent(variant: dict, variant_path: str):
    ...
    # Action width follows chunk_len (the actor's full predicted chunk), not
    # query_freq (the re-query cadence). Matches YamGymEnv.action_space.
    sample_act = np.zeros((1, chunk_len, action_dim), dtype=np.float32)
    ...
    if algo == "residual_parl":
        ...
        for k in (
            "parl_num_samples", "parl_num_elites", "parl_num_grad_steps", "parl_step_size",
            "b_o_n", "grad_a_q", "parl_a_star_delta_clip_norm",
        ):
            if k in variant:
